...
class Network:
    ...
    def get_spike_train_list(self):
        spike_train_dict = self.get_spktrains()
        n_neurons = self.n_neurons
        spk_trains = [[] for _ in range(self.n_neurons)]

        for neuron_key in range(1, n_neurons + 1):
            if neuron_key in spike_train_dict:
                times = spike_train_dict[neuron_key]
    ...
    def evaluate(
        ...
        if nest.Rank() == 0:
            ...
            helpers.boxplot(self.data_path, self.net_dict["populations"], title)
    ...
    def __create_neuronal_populations(self):
        ...
        for i in np.arange(self.num_pops):
            ...
            locs_thispop = locs_thispop[
                np.argsort(locs_thispop[:, 1]), :
            ]  # sort by depth
            self.neuron_locations = np.concatenate(
                [self.neuron_locations, locs_thispop], axis=0
            )
            population.set(
